...
# will make the logical lambda expressions from a tree. I'm not completely sure how lambda expressions work, so I gave it my best try. It doesn't run correctly when I do the dialogue though, so I'm sure it's not fully functional.
def sem(tr):
    """translates a syntax tree into a logical lambda expression (in string form)"""
    rule = top_level_rule(tr)
   
    #basically tried to follow the rules for the examples through all of it
    if (tr.label() == 'P'):
        return tr[0][0]
    elif (tr.label() == 'N'):
    ...
    elif (rule == 'QP -> DO NP T'):                   # these last three are the harder ones, T verbs all mean use of 'exists'
