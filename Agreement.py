...
def N_phrase_num(tr):
    ...
    elif (tr.label() == 'AN'):   # AN has two options
        if (tr[0].label() == 'N'): # first starts with N, only has one part
            return N_phrase_num(tr[0])
        else:
            return N_phrase_num(tr[1]) # other has two parts, but only second matters
    elif (tr.label() == 'NP'):  # NP can start with P, AR, or Nom
        if (tr[0].label() == 'P'):
            return N_phrase_num(tr[0]) # only has one part
        elif (tr[0].label() == 'Nom'): 
            return N_phrase_num(tr[0]) # only has one part too
        elif (tr[0].label() == 'AR'):
            return N_phrase_num(tr[1]) # only second part matters
    else:
        return 's' # base case: return singular
...
def V_phrase_num(tr):
    ...
    if (tr.label() == 'T' or tr.label() == 'I'):
        ...
        return tr[0][1]
    elif (tr.label() == 'QP'): # two options
        if tr[0].label() == 'VP': 
            return V_phrase_num(tr[0])
        else:
            return '' # not just verb phrase
    elif (tr.label() == 'Rel'):     #two options
        if tr[0].label() == 'WHO': # only second part matters
            return V_phrase_num(tr[1])
        else:
            return '' # rest has noun phrase
    else:
        return '' # base case
...
def check_node(tr):
    ...
    # for all of these, if there is something for V/N_phrase_num to check, it is done. If not, decide if verb/noun should be singular or plural based off of context.
